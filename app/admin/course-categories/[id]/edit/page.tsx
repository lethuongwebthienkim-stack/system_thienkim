  ...
  if (categoryData === null) {
    return (
      <div className="py-12 text-center">
        ...
        <Link href="/admin/course-categories" className="mt-2 inline-block text-indigo-600 hover:underline">Quay lại danh sách</Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6 pb-20">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Sửa danh mục khóa học</h1>
            <Link href="/admin/course-categories" className="text-sm text-indigo-600 hover:underline">Quay lại danh sách</Link>
          </div>
        </div>
      </div>
      ...
      <Card className="mx-auto max-w-md md:mx-0">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-3 rounded-b-lg border-t border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800">
            <Button type="button" variant="ghost" onClick={() => { router.push('/admin/course-categories'); }}>Hủy bỏ</Button>
