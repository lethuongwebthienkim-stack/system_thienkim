      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Danh mục tài nguyên</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">Quản lý phân loại tài nguyên</p>
            </div>
          </div>
          <Link href="/admin/resource-categories/create">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <SortableHeader label="Số tài nguyên" sortKey="count" sortConfig={sortConfig} onSort={(key) => { setSortConfig((prev) => ({ direction: prev.key === key && prev.direction === 'asc' ? 'desc' : 'asc', key })); }} className="text-center" />
                ...
                <TableRow key={category.id} className={selectedIds.includes(category.id) ? 'bg-indigo-500/5' : ''}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Link href={`/admin/resource-categories/${category.id}/edit`}><Button variant="ghost" size="icon"><Edit size={16} /></Button></Link>
