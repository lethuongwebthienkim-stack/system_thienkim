...
- **Nguyên nhân**: Khi lưu cài đặt, code ở frontend đã lọc bỏ thuộc tính `storageId` của các ảnh này, chỉ giữ lại `url`, `link`, `name`. Vì không lưu `storageId` vào database nên hệ thống dọn dẹp (cleanup cron job) của backend nghĩ rằng các ảnh này không được sử dụng ở đâu cả và đã xóa chúng khỏi bộ nhớ Convex Storage để giải phóng băng thông.
- **Cách sửa**: Thêm `storageId` vào danh sách thuộc tính được tải lên, được so sánh, và được lưu xuống database của cả 3 component (Partners, Gallery, TrustBadges) ở cả trang tạo mới (create) và trang chỉnh sửa (edit).
...
## IV. Proposal (Đề xuất)
1. Cập nhật logic load dữ liệu (`useEffect` ban đầu) ở các trang Edit để map và giữ lại `storageId` từ config cũ (nếu có).
2. Cập nhật hàm `normalizeItemsForCompare` hoặc logic so sánh (nếu có) để bao gồm `storageId` nhằm tránh việc so sánh bị sai lệch hoặc không nhận diện được thay đổi khi đổi ảnh.
3. Cập nhật logic submit (`handleSubmit` hoặc `onSubmit`) ở tất cả 6 trang (create & edit của Partners, Gallery, TrustBadges) để map cả `storageId` vào cấu trúc `items` gửi lên Convex backend.
...
1. **Sửa**: [app/admin/home-components/partners/[id]/edit/page.tsx](file:///e:/NextJS/job/job_from_system_vietadmin/system_thienkim/app/admin/home-components/partners/[id]/edit/page.tsx)
   - Sửa: Dòng load dữ liệu trong `useEffect` để map `storageId`.
